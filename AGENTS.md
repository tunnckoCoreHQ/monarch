...
- Open pull requests with `gh pr create`, then run `gh pr merge --auto --squash` on them. GitHub merges when the required `check` and `test` checks pass, and that merge triggers the master workflows. Dependabot and Socket Optimize PRs get auto-merge from their own workflows using the `OLSTENLARCK_HQ_PAT` secret.
