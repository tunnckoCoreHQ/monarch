...
Open [preview/cats-12.html](./preview/cats-12.html) for the full collection. The gallery embeds all 1,000 SVGs and displays 36 per page. The preview command writes individual SVGs to `preview/rendered/`; the golden test image is tracked separately in `test/fixtures/Mews.svg`. The original artwork is in [references](./references/).
