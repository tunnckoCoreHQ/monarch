...
| -------- | -------------------- | ----------------------- | -------------------- | ------------------------------------ |
| `master` | `triad-auth-nightly` | `wrangler.nightly.toml` | `triad-auth-nightly` | `https://triad-auth-nightly.wgw.lol` |
| `stable` | `triad-auth`         | `wrangler.toml`         | `triad-auth`         | `https://triad-auth.wgw.lol`         |
...
Fill `.dev.vars` with local values. `vp run dev` uses `wrangler.toml` bindings against local D1 storage.
...
| Script                  | What it does                                                                                |
| ----------------------- | ------------------------------------------------------------------------------------------- |
| `vp run build`          | Astro build against `wrangler.toml`                                                         |
| `vp run build:nightly`  | Astro build against `wrangler.nightly.toml`                                                 |
| `vp run deploy`         | `wrangler d1 migrations apply DB --remote -c wrangler.toml`, then `wrangler deploy`         |
| `vp run deploy:nightly` | `wrangler d1 migrations apply DB --remote -c wrangler.nightly.toml`, then `wrangler deploy` |
...
```sh
vp exec wrangler secret put <NAME> -c wrangler.nightly.toml
vp exec wrangler secret put <NAME> -c wrangler.toml
```
